...
def pvp_infer(model, device, all_loader):
    ...
    with torch.no_grad():
        for batch_idx, (x0, x1, labels, class_labels0, class_labels1) in enumerate(all_loader):
            ...
            class_labels0_device = class_labels0.to(device)
            class_labels1_device = class_labels1.to(device)
            ...
            for i in range(test_num):
                ...
                if class_labels0_device[i] == class_labels1_device[idx[0]]:
...
def pdp_infer(model, device, all_loader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x0, x1, labels, class_labels0, class_labels1, mask) in enumerate(all_loader):
            test_num = len(labels)
            x0, x1, labels, mask = x0.to(device), x1.to(device), labels.to(device), mask.to(device)
            x0 = x0.view(x0.size()[0], -1)
            x1 = x1.view(x1.size()[0], -1)
        ...
        for batch_idx, (x0, x1, labels, class_labels0, class_labels1, mask) in enumerate(all_loader):
            ...
            if setting != 0:
                ...
                for i in range(test_num):
                    ...
                    if len(idx1) != 0 and len(idx0) == 0:  # i-th sample in view 1 is missing
                        avg_fill = h1[idx1[0:fill_num], :].sum(dim=0) / fill_num
                        cnt += (class_labels1_device[idx1[0:fill_num]] == class_labels1_device[i]).sum()
                        missing_cnt += 1
                        recover_out0[i, :] = h0[i, :]
                        recover_out1[i, :] = avg_fill  # missing
                    elif len(idx0) != 0 and len(idx1) == 0:
                        avg_fill = h0[idx0[0:fill_num], :].sum(dim=0) / fill_num
                        cnt += (class_labels0_device[idx0[0:fill_num]] == class_labels0_device[i]).sum()
                        missing_cnt += 1
